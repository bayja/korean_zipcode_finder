...
KoreanZipcodeFinder = (function () {
  ...
  function KoreanZipcodeFinder() {
    ...
    $(document).on("hide.bs.modal", ".korean_zipcode_finder_modal_wrapper #zipcodeModalCenter", this.close_modal_handler);
  ...
  KoreanZipcodeFinder.prototype.zipcode_selected_handler = function (e) {
    ...
    var address_02 = $(e.target).attr('data-address02');
    ...
    this.current_address_area.find(".address_02").val(address_02);

    this.close_modal_handler(e);
  };

  KoreanZipcodeFinder.prototype.paging_handler = function (e) {
    var countPerPage = $(e.target)[0].href.split('?').pop().split("&").find(function (ele) {
      return ele.split('=')[0] == 'countPerPage'
    });
    var currentPage = $(e.target)[0].href.split('?').pop().split("&").find(function (ele) {
      return ele.split('=')[0] == 'page'
    })
